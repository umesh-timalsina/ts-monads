...
describe('Maybe', function () {
    ...
    it('should return Ok/Err result for okOr with some/none respectively', () => {
        const maybeNone = new None<string>();
        const okOrResultNone = maybeNone.okOr(new Error('Value should be a string, found none'));
        expect(() => okOrResultNone.unwrap()).toThrowError('');

        const maybeSome = new Some<string>('My String');
        const okOrResultSome = maybeSome.okOr(new Error('Value should be a string, found none'));
    ...
    it('should throw return Ok/Err result from a function for okOrElse with some/none respectively', () => {
        const maybeNone = new None<string>();
        const okOrElseResultNone = maybeNone.okOrElse(() => new Error('Value should be a string, found none'));
        expect(() => okOrElseResultNone.unwrap()).toThrowError('');

        const maybeSome = new Some<string>('My String');
        const okOrElseResultSome = maybeSome.okOrElse(() => new Error('Value should be a string, found none'));
    ...
    it('should return some/none of new types for mapAsync with some/none', async () => {
        const maybeNone = new None<number>();
        const mappedNone = await maybeNone.mapAsync(async (val) => val.toString());
        expect(mappedNone).toEqual(new None<string>());

        const maybeSome = new Some<number>(25);
        const mappedSome = await maybeSome.mapAsync(async (val) => val.toString());
        expect(mappedSome).toEqual(new Some('25'));
    });

    it('should return default/transformed values of new types for mapOr with some/none', () => {
        const maybeNone = new None<number>();
        const mappedNone = maybeNone.mapOr('my custom string', (val) => val.toString());
        expect(mappedNone).toEqual('my custom string');

        const maybeSome = new Some<number>(25);
        const mappedSome = maybeSome.mapOr('my custom string', (val) => val.toString());
        expect(mappedSome).toEqual('25');
    });

    it('should return default/transformed values of new types for mapAsyncOr with some/none', async () => {
        const maybeNone = new None<number>();
        const mappedNone = await maybeNone.mapAsyncOr('my custom string', async (val) => val.toString());
        expect(mappedNone).toEqual('my custom string');

        const maybeSome = new Some<number>(25);
        const mappedSome = await maybeSome.mapAsyncOr('my custom string', async (val) => val.toString());
        expect(mappedSome).toEqual('25');
    });

    it('should return default/transformed values of new types from function for mapOrElse with some/none', () => {
        const maybeNone = new None<number>();
        const mappedNone = maybeNone.mapOrElse(() => 'my custom string', (val) => val.toString());
        expect(mappedNone).toEqual('my custom string');

        const maybeSome = new Some<number>(25);
        const mappedSome = maybeSome.mapOrElse(() => 'my custom string', (val) => val.toString());
        expect(mappedSome).toEqual('25');
    });

    it('should return default/transformed values of new types from function for mapAsyncOrElse with some/none', async () => {
        const maybeNone = new None<number>();
        const mappedNone = await maybeNone.mapAsyncOrElse(() => 'my custom string', async (val) => val.toString());
        expect(mappedNone).toEqual('my custom string');

        const maybeSome = new Some<number>(25);
        const mappedSome = await maybeSome.mapAsyncOrElse(() => 'my custom string', async (val) => val.toString());
        expect(mappedSome).toEqual('25');
    });
